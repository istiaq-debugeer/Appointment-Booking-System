...
2. Install dependencies:
   ...
   pip install -r requirements.txt   3. Set up environment variables:
      - Copy the attached `.env.example` file to `.env` and update the values as needed.
   ```
...
5. (Optional) Start Celery worker:
   ```sh
   Before Run selery add redis_url in env
   celery -A src.core.celery worker --loglevel=info
   (Optional) Start Celery Beats Scheduler:
